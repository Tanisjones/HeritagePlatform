...
**The URL carries the city.** `/` is the platform gateway (city cards); public content lives under `/<citySlug>/…` (`/riobamba`, `/tarragona/explore`) with `/all/…` as the cross-city mode (no `X-City`); account/management pages (`/login`, `/dashboard`, `/moderation*`, `/teach*`, …) stay unprefixed. Two layers of city state, deliberately separate: **chosen** (`hp_city` in localStorage — what the user picked in the switcher/gateway, shared across tabs, scopes the unprefixed pages) and **URL** (in-memory, per-tab, set by `CityShellView`, released on leaving the shell). `activeCitySlug` is URL-then-chosen and drives `X-City` via `setRequestCity()`; browsing a city URL never writes `hp_city`, so two tabs on two cities don't fight and a shared cross-city link doesn't re-point someone's account pages.

`CityShellView` validates the slug against the catalog before rendering children (unknown → 404, no catalog → error+retry, never "trust it and send a bogus `X-City`"). Route `meta` declares city behavior: `cityScope: 'generic' | 'entity'` (what a city switch does) and `requiresConcreteCity` on write routes (`contribute`, `routes/new`, `routes/:id/edit`) — `/all/contribute` would otherwise file the record under `City.get_default()`. Legacy unprefixed URLs redirect via `frontend/src/router/cityContext.ts` (derived from the shell's `children`, so they can't drift); content links use `useCityPath()` — `cityPath()` for the browsing context, `cityPathFor(slug, …)` when a record carries its own city. Reserved first segments are rejected by `validate_city_slug`, enforced in `City.save()` (field validators alone never run on `objects.create()`/seeders).
...
$B python manage.py seed_education <slug> [--author-email x@y.z]  # shared edu content (EducationalResource + LessonPlan) from data/education/<slug>.py (idempotent; run seed_city + seed_curriculum first)
